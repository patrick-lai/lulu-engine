...
class Recorder extends Component {
  ...
  render() {
    ...
    let buttonText, buttonClass = ['Recorder-button'], audioButtons;
    let clickHandler;
    if(this.state.audio) {
      ...
      audioButtons = [
        <button key="remove" className="Recorder-remove" onClick={this.removeAudio.bind(this)}>{strings.remove}</button>
      ];

      if(this.props.download) {
        audioButtons.push(
          <button key="download" className="Recorder-download" onClick={this.downloadAudio.bind(this)}>{strings.download}</button>
        );
      }
    } else {
